...
定位
----
该 demo 希望“拿来即跑”，但 `web-demo-video-synthesis` 的默认 `tts_build_workspace.py` 需要阿里云 ISI 的
`key.json`。为了让演示无需任何密钥，本脚本使用 macOS 自带的 `say` 作为 TTS：

- 分段生成 `segment_audio/seg_000.wav` ...
- 计算每段时长，并按该 skill 的严格 schema 生成 `timeline/timeline.json`
- 可选调用该 skill 的 `mix_audio_from_timeline.py` 输出 `audio/timeline_audio.mp3`

协作/可审计原则
---------------
- `workspace_meta.json` 固化 cues + TTS 参数签名，防止静默复用旧音频（默认“正确失败”）。
- 任意段落合成失败会直接报错，便于定位 seg_index。
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--voice", default="Tingting", help="macOS say 音色（例如 Tingting）")
    parser.add_argument("--rate", type=int, default=None, help="say 语速（可选，单位为 words per minute）")
...
def build_timeline(
    *,
    ...
    """复用该 skill 的“段长 + gap 驱动”时间轴规则，输出严格 schema。"""
...
def run_mix_audio_from_timeline(*, skill_dir: Path, timeline_json: Path, out_mp3: Path) -> None:
    mix_py = (skill_dir / "scripts" / "mix_audio_from_timeline.py").resolve()
...
def main() -> None:
    ...
    if args.mix_audio:
        ...
        run_mix_audio_from_timeline(skill_dir=skill_dir, timeline_json=timeline_path, out_mp3=out_mp3)
